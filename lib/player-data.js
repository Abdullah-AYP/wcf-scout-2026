...
const NON_SELECTABLE_STATUSES = new Set(["transferred", "eliminated", "unavailable", "withdrawn", "injured", "suspended"]);
...
async function loadRemoteData(baseUrl) {
  ...
  const stale = isClearlyStale(rounds, players);
  if (stale && process.env.WCF_ALLOW_STALE_DATA !== "true") {
  ...
  const diagnostics = buildDiagnostics(players, normalizedPlayers);
  logDiagnostics(diagnostics, officialSource ? "FIFA official" : cleanBase);
...
function loadSampleData(warning) {
  ...
  const diagnostics = buildDiagnostics(players, players);
  logDiagnostics(diagnostics, "sample data");
  return {
    source: "sample",
    sourceLabel: "Demo player data",
    warning: warning || "Live FIFA data is unavailable, so demo data is shown.",
    stale: true,
    demoData: true,
...
function normalizePlayers(players, teamsById, rounds) {
  return players.map((player) => {
    ...
    return {
      ...
      position: POSITION_MAP[player.position] || POSITION_MAP[player.positionName] || "",
      price,
...
function buildDiagnostics(rawPlayers, normalizedPlayers) {
  const raw = Array.isArray(rawPlayers) ? rawPlayers : [];
  const normalized = Array.isArray(normalizedPlayers) ? normalizedPlayers : [];
  ...
  const statusBreakdown = raw.reduce((counts, player) => {
    const status = normalizeStatusValue(player.status);
    counts[status] = (counts[status] || 0) + 1;
    return counts;
  }, {});

  const selectablePlayers = normalized.filter(isSelectableFantasyPlayer);
  const unavailablePlayers = normalized.filter((player) => !isSelectableFantasyPlayer(player));

  return {
    rawRecordCount: raw.length,
    uniquePlayerIdCount: idCounts.size,
    duplicateIdCount: duplicateIds.length,
    duplicateIds: duplicateIds.slice(0, 12),
    missingTeamCount: normalized.filter((player) => !player.team || player.team === "Unknown").length,
    invalidPositionCount: raw.filter((player) => !POSITION_MAP[player.position] && !POSITION_MAP[player.positionName]).length,
    missingPriceCount: raw.filter((player) => player.price === undefined && player.cost === undefined && player.value === undefined).length,
    statusBreakdown,
    unavailableCount: unavailablePlayers.length,
    selectablePlayerCount: selectablePlayers.length,
    selectableStatusBreakdown: selectablePlayers.reduce((counts, player) => {
      const status = normalizeStatusValue(player.status);
      counts[status] = (counts[status] || 0) + 1;
      return counts;
    }, {})
...
function isSelectableFantasyPlayer(player) {
  const status = normalizeStatusValue(player.status);
  return Boolean(player.id)
    && Boolean(player.team)
    && isValidPosition(player.position)
    && Boolean(player.price)
    && !player.isEliminated
    && !NON_SELECTABLE_STATUSES.has(status);
}
...
function rawPlayerId(player) {
  return String(player?.id ?? player?.feedId ?? player?.optaId ?? player?.name ?? "missing-id");
}
...
function logDiagnostics(diagnostics, label) {
  if (process.env.NODE_ENV === "production" || process.env.VERCEL_ENV === "production") return;
  const statuses = Object.entries(diagnostics.statusBreakdown)
    .map(([status, count]) => `${status}:${count}`)
    .join(", ");
  console.info(
    `[WCF data] ${label}: ${diagnostics.rawRecordCount} records, ${diagnostics.uniquePlayerIdCount} unique IDs, `
    + `${diagnostics.duplicateIdCount} duplicate IDs, ${diagnostics.selectablePlayerCount} selectable, `
    + `${diagnostics.unavailableCount} unavailable, `
    + `${diagnostics.missingTeamCount} missing team, ${diagnostics.invalidPositionCount} invalid position, `
    + `${diagnostics.missingPriceCount} missing price. Statuses: ${statuses || "none"}`
  );
}
