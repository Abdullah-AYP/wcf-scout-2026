const MAX_ANALYZE_BODY_BYTES = 48_000;
const ANALYZE_COOLDOWN_MS = 10_000;
const ANALYZE_WINDOW_MS = 60_000;
const ANALYZE_MAX_PER_WINDOW = 6;
const ANALYZE_HOUR_MS = 60 * 60_000;
const ANALYZE_MAX_PER_HOUR = 40;
...
function prepareAnalyzeRequest(req, input, options = {}) {
  assertBodySize(req, input);
  enforceAnalyzeRateLimit(req, options.now);
...
function enforceAnalyzeRateLimit(req, now = Date.now()) {
  if (process.env.WCF_DISABLE_RATE_LIMIT === "true") return;

  const ip = clientIp(req);
  const entry = rateLimitStore.get(ip) || { requests: [], cooldownUntil: 0 };
  entry.requests = entry.requests.filter((timestamp) => now - timestamp < ANALYZE_HOUR_MS);

  if (entry.cooldownUntil > now) {
    const retryAfter = Math.ceil((entry.cooldownUntil - now) / 1000);
    throw publicError(429, "COOLDOWN", `Slow down a little. Try again in ${retryAfter} seconds.`, retryAfter);
  }

  const recentWindowCount = entry.requests.filter((timestamp) => now - timestamp < ANALYZE_WINDOW_MS).length;
  if (recentWindowCount >= ANALYZE_MAX_PER_WINDOW) {
    const oldestWindowRequest = entry.requests.find((timestamp) => now - timestamp < ANALYZE_WINDOW_MS);
    const retryAfter = Math.max(1, Math.ceil((ANALYZE_WINDOW_MS - (now - oldestWindowRequest)) / 1000));
    throw publicError(429, "RATE_LIMITED", `Too many scout reports from this connection. Try again in ${retryAfter} seconds.`, retryAfter);
  }

  if (entry.requests.length >= ANALYZE_MAX_PER_HOUR) {
    const retryAfter = Math.max(1, Math.ceil((ANALYZE_HOUR_MS - (now - entry.requests[0])) / 1000));
    throw publicError(429, "HOURLY_LIMIT", "This connection used its hourly AI scout limit. Please try again later.", retryAfter);
  }

  entry.requests.push(now);
  entry.cooldownUntil = now + ANALYZE_COOLDOWN_MS;
...
function pruneRateLimitStore(now) {
  ...
  for (const [ip, entry] of rateLimitStore.entries()) {
    const hasRecent = entry.requests.some((timestamp) => now - timestamp < ANALYZE_HOUR_MS);
    if (!hasRecent && entry.cooldownUntil <= now) rateLimitStore.delete(ip);
  }
}
