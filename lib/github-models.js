...
async function analyzeWcfRequest({ tool, payload }) {
  ...
  if (!response.ok) {
    const message = authErrorMessage(response.status) || modelErrorMessage(data) || `GitHub Models request failed with ${response.status}.`;
...
function shouldRetryWithDefaultModel(response, data, model) {
  ...
  return response.status === 400 && message.includes("unknown model");
}
...
function authErrorMessage(status) {
  if (status === 401) {
    return "GitHub Models rejected the token. Add a valid GITHUB_MODELS_TOKEN with Models access in your local .env or Vercel environment variables.";
  }
  if (status === 403) {
    return "GitHub Models denied access. Check that your GitHub PAT has Models access and that your account can use GitHub Models.";
  }
  return "";
}
